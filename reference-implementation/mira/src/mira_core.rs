...
///
/// Mira görevleri koordine eder, Rasterast raporlarını
/// görevlere bağlar, Müdebbir kararlarını kaydeder ve
/// doğrulanmış akademik çıktıları saklar.
///
/// Nihai karar yetkisi Müdebbir’e aittir.
...
impl MiraCore {
    ...
    /// Müdebbir onayı gerektiren yüksek riskli
    /// akademik görev oluşturur.
    ...
    /// Mira tarafından saklanan akademik çıktıları
    /// salt okunur olarak döndürür.
    pub fn academic_outputs(
        &self,
    ) -> &[TaskAcademicOutput] {
    ...
    /// Planlanan görevi çalışma aşamasına geçirir.
    ...
    /// Çalışan görevi Rasterast doğrulaması bekleme
    /// aşamasına geçirir.
    pub fn await_rasterast(
        &mut self,
        task_id: Uuid,
    ) -> bool {
        let Some(task) = self.find_task_mut(task_id) else {
        ...
        task.update_status(
            MiraTaskStatus::AwaitingRasterast,
        );
...
/// Rasterast raporunu ilgili göreve bağlar.
    ///
    /// Rapor yalnızca Rasterast değerlendirmesi bekleyen
    /// göreve eklenebilir.
    ...
    /// Rasterast raporuna dayanarak Müdebbir’e
    /// sunulacak Mira önerisini oluşturur.
    pub fn create_recommendation(
        ...
        let requires_mudebbir_approval =
            task.requires_mudebbir_approval;
    ...
    /// Mira tarafından oluşturulan önerileri
    /// salt okunur olarak döndürür.
    ...
    /// Müdebbir onayını bekleyen görevi onaylar
    /// ve kararı kalıcı olarak kaydeder.
    pub fn approve_task(
        &mut self,
        task_id: Uuid,
    ) -> bool {
 ...
 /// Müdebbir onayını bekleyen görevi reddeder
    /// ve kararı kalıcı olarak kaydeder.
    pub fn reject_task(
        &mut self,
        task_id: Uuid,
    ...
    /// Akademik çıktıyı onu üreten görev kimliğiyle
    /// birlikte saklar.
    ...
    /// Belirtilen göreve ait akademik çıktıyı bulur.
    ...
    /// Müdebbir tarafından onaylanmış görev için
    /// akademik üretim hattını çalıştırır.
    ///
    /// Çıktı görev kimliğiyle saklanır ve görev
    /// tamamlanmış duruma geçirilir.
    pub fn run_verified_analysis(
        ...
        task.update_status(
            MiraTaskStatus::Completed,
        );

        true
    }

    /// Müdebbir tarafından verilen bütün kararları
    /// salt okunur olarak döndürür.
    pub fn mudebbir_decisions(
        &self,
    ) -> &[MudebbirDecisionRecord] {
        &self.mudebbir_decisions
    }

    /// Belirtilen göreve ait en son Müdebbir
    /// kararını bulur.
    ...
    /// Onaylanan akademik görevin akademik üretim
    /// hattını başlatmaya uygun olup olmadığını
    /// bildirir.
    ...
    /// Kimliğine göre görevi değiştirilebilir
    /// olarak bulur.
    pub fn find_task_mut(
        &mut self,
        task_id: Uuid,
    ) -> Option<&mut MiraTask> {
        self.tasks
...
mod tests {
    ...
    fn prepare_task_for_mudebbir(
        ...
        let report =
            verified_rasterast_report(task_id);

        assert!(
            mira.attach_rasterast_report(report)
        );
        ...
        assert!(mira.create_recommendation(
            ...
            vec![
                "Akademik üretim için uygun bulundu."
                    .to_string(),
            ],
            vec![
                "Taslak üretimle sınırlı tutulabilir."
                    .to_string(),
            ],
    ...
    #[test]
    fn mira_registers_academic_task_with_approval() {
        let mut mira = MiraCore::new();
        ...
        let task = mira
            .find_task(task_id)
            .expect(
                "Akademik görev kayıtlı olmalıdır.",
            );

        assert_eq!(
            task.status,
            MiraTaskStatus::Created,
        );
    ...
    #[test]
    fn mira_moves_created_task_to_planning() {
        let mut mira = MiraCore::new();
...
#[test]
    fn mira_moves_planning_task_to_running() {
        ...
        assert_eq!(
            task.status,
            MiraTaskStatus::Running,
        );
    }

    #[test]
    fn running_task_can_await_rasterast() {
        let mut mira = MiraCore::new();
    ...
    #[test]
    fn mira_attaches_rasterast_report() {
        let mut mira = MiraCore::new();
        ...
        assert_eq!(
            mira.rasterast_reports.len(),
            1,
        );

        assert_eq!(
            mira.rasterast_reports[0].task_id,
            task_id,
        );
    }

    #[test]
    fn mira_creates_recommendation_from_report() {
        let mut mira = MiraCore::new();
        ...
        let report = RasterastReport {
            ...
            contradictions: Vec::new(),
            risks: vec![
                "Kaynaklar yayın öncesinde yeniden kontrol edilmelidir."
                    .to_string(),
            ],
            requires_mudebbir_decision: true,
            created_at: Utc::now(),
        ...
        assert!(mira.create_recommendation(
            task_id,
            "Hebûn çalışması akademik üretime uygundur.",
            vec![
                "Hebûn kuramının matematiksel sunumunu güçlendirir."
                    .to_string(),
            ],
            vec![
                "Makale önce taslak olarak hazırlanabilir."
                    .to_string(),
            ],
            "Müdebbir onayından sonra akademik üretimi çalıştır.",
        ));

        assert_eq!(
            mira.recommendations().len(),
            1,
        );
        ...
        let recommendation = mira
            ...
            .expect(
                "Göreve ait Mira önerisi bulunmalıdır.",
            );
    ...
    #[test]
    fn verified_task_can_await_mudebbir() {
        let mut mira = MiraCore::new();

        let task_id =
            prepare_task_for_mudebbir(&mut mira);
...
#[test]
    fn mudebbir_approves_awaiting_task() {
        let mut mira = MiraCore::new();
        ...
        let decision = mira
            ...
            .expect(
                "Göreve ait Müdebbir kararı bulunmalıdır.",
            );
    ...
    #[test]
    fn mudebbir_rejects_awaiting_task() {
        let mut mira = MiraCore::new();
    ...
    fn approved_task_can_start_academic_pipeline() {
        ...
        assert!(
            mira.can_start_academic_pipeline(
                task_id
            )
        );
    }
    ...
    #[test]
    fn unapproved_task_cannot_start_academic_pipeline() {
        let mut mira = MiraCore::new();
    ...
    fn hebun_academic_task_completes_end_to_end() {
        ...
        let stored_output = mira
            ...
            .last()
            .expect(
                "Mira akademik çıktıyı görevle birlikte saklamalıdır.",
            );

        assert_eq!(
            stored_output.task_id,
            task_id,
        );
        ...
        let task_output = mira
            ...
            .expect(
                "Göreve ait akademik çıktı bulunmalıdır.",
            );

        assert!(
            task_output.is_ready_for_publication()
        );

        let task = mira
            .find_task(task_id)
            .expect(
                "Akademik görev Mira içinde kayıtlı olmalıdır.",
            );
        ...
        let recommendation = mira
            .recommendation_for_task(task_id)
            .expect(
                "Hebûn görevi için Mira önerisi bulunmalıdır.",
            );
        ...
        let decision = mira
            .mudebbir_decision_for_task(task_id)
            .expect(
                "Hebûn görevi için Müdebbir kararı bulunmalıdır.",
            );
